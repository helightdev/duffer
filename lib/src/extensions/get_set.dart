...
extension GetSetExtension on ByteBuf {
  ...
  int getInt64(int index, [Endian? endian]) => getByteData(index, 8).getInt64(0, endian ?? kEndianness);

  void setInt32(int index, int value, [Endian? endian]) =>
      setByteData(index, 4).setInt32(0, value, endian ?? kEndianness);
  int getInt32(int index, [Endian? endian]) => getByteData(index, 4).getInt32(0, endian ?? kEndianness);

  void setInt16(int index, int value, [Endian? endian]) =>
      setByteData(index, 2).setInt16(0, value, endian ?? kEndianness);
  int getInt16(int index, [Endian? endian]) => getByteData(index, 2).getInt16(0, endian ?? kEndianness);

  void setUint64(int index, int value, [Endian? endian]) =>
      setByteData(index, 8).setUint64(0, value, endian ?? kEndianness);
  int getUint64(int index, [Endian? endian]) => getByteData(index, 8).getUint64(0, endian ?? kEndianness);

  void setUint32(int index, int value, [Endian? endian]) =>
      setByteData(index, 4).setUint32(0, value, endian ?? kEndianness);
  int getUint32(int index, [Endian? endian]) => getByteData(index, 4).getUint32(0, endian ?? kEndianness);

  void setUint16(int index, int value, [Endian? endian]) =>
      setByteData(index, 2).setUint16(0, value, endian ?? kEndianness);
  int getUint16(int index, [Endian? endian]) => getByteData(index, 2).getUint16(0, endian ?? kEndianness);

  void setFloat64(int index, double value, [Endian? endian]) =>
      setByteData(index, 8).setFloat64(0, value, endian ?? kEndianness);
  double getFloat64(int index, [Endian? endian]) => getByteData(index, 8).getFloat64(0, endian ?? kEndianness);

  void setFloat32(int index, double value, [Endian? endian]) =>
      setByteData(index, 4).setFloat32(0, value, endian ?? kEndianness);
  double getFloat32(int index, [Endian? endian]) => getByteData(index, 4).getFloat32(0, endian ?? kEndianness);

  void setLPString(int index, String string, [Encoding? encoding, Endian? endian]) {
